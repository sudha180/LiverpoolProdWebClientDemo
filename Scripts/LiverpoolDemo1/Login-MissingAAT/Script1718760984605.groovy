...
def responseText = AuthResponse.getResponseText()
...
WS.verifyResponseStatusCode(AuthResponse, 200, FailureHandling.STOP_ON_FAILURE)

anonymousCartId = WS.getElementPropertyValue(AuthResponse, 'result.anonymousCartId')

println(anonymousCartId)

println(' Testcase got passed for the missing AAT')
